...
/** Common academic degrees / qualifications. Used as suggestions in the
 *  Education tab — backed by AntD AutoComplete so HR can pick from the
 *  list or type a custom one (an explicit "Others" option isn't needed
 *  since AutoComplete already accepts free text). Keep this list curated
 *  rather than exhaustive — exotic degrees end up as free text. */
...
/** Common fields of study. Same UX pattern as DEGREE_OPTIONS — suggested
 *  values via AutoComplete, with free text accepted for anything not on
 *  the list. */
...
/** Common designations / job titles — shown as AutoComplete suggestions
 *  in the onboarding wizard. Free text is accepted for anything not in
 *  the list, so we don't need an explicit "Others" option. */
