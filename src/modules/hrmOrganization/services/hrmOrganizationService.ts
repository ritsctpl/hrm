...
export class HrmOrganizationService {
  ...
  static async fetchAllOrganizations(): Promise<OrganizationSummary[]> {
    ...
    const mapped = (raw as Array<Record<string, unknown>>)
      .map((o) => {
        // Use the actual organizationId (= site code) from the backend response.
        // Falling back to a legalName-derived slug caused org-switcher IDs to
        // mismatch the site codes stored in RBAC/module/assignment collections,
        // so super-admin users could not switch to orgs with multi-word legal names.
        const orgId = String(o.organizationId ?? o.site ?? '').trim();
        const legalName = String(o.legalName ?? o.organizationName ?? o.name ?? '').trim();
        const id = orgId || legalName.replace(/\s+/g, '_');
        return {
          organizationId: id,
